...
**Current refinement — saved attempts and simpler landing:** persist the real daily round in localStorage, retaining its original deadline, confirmed guesses and pending marker. Serialize cross-tab writes with Web Locks and reject stale confirmations. Resume on refresh/reopen/focus; elapsed time includes time away. Preserve completed results and remove Dream again. Fail clearly if saved data/storage is unavailable, without silently resetting. Clearing site data permits replay; no account or server anti-cheat. Keep dev review/New day in memory, independent of the real saved day. Add click-to-toggle landing zoom with no guessing UI, bold remember, a Start action reachable above the fold on narrow/short screens, artwork-adjacent close controls, and title-case pair labels. No new dependencies, backend, or deployment are included.

Seven persistence tests supplement the existing 24 tests: pending/deadline restore, time-away expiry, immutable five-guess results, stale confirmations, latest-storage reads/day isolation, damaged records, and failed writes. Browser verification includes reload/cross-tab state and result retention; physical phone gesture acceptance remains separate.

**Current authorized refinement — full-screen inspection:** replace inline zoom/pan with an explicit fitted image viewer. Landing: click/tap/hold or Expand to inspect without starting play. Game: short tap marks; stationary hold or Expand opens; zoom/pan and Remember work inside the viewer. Close by X/Escape, contain/restore focus and preserve pending guesses, timer and page position. Never submit on hold, drag, pinch or close; no version-swap shortcut. Timer expiry/final confirmation dismisses the viewer. Retain result overlays with Hide markers inside inspection. Fit the pair side by side on wider screens/portrait tablets; stack only narrow portrait screens up to 40rem. Test at 320px, 390px, portrait tablet, landscape phone and desktop; verify timer expiry while open, one confirmation per guess, no accidental drag selections, zoomed coordinate mapping, focus return, and no overflow. Physical long-press/pinch acceptance remains a separate real-device check. These interaction rules replace the inline-zoom wording in the earlier numbered scope below. No deployment is authorized by this refinement alone.
...
**Current authorized refinement:** New online rooms default to Word of the Day with ranked recognition awards. Personal dream clues remain selectable at creation with their original scoring; invitations clearly show the creator's fixed mode. Add clue examples, explicit six-Dream preparation guidance and a reduced-motion-aware decorative page cue. Preserve existing rooms through migration. See the confirmed room-mode rules in GAME_DESIGN.md and the connected plan. This refinement does not authorize matchmaking, a leaderboard or a new preparation schedule; stop for user testing before deployment.
...
Milestone 0.2.0 has a [concrete technical design](PROTOTYPE_0_2_TECHNICAL_DESIGN.md). Shared play, scheduling and lifecycle through 0.2.7 are approved by the instruction to proceed. Milestone 0.2.8 is in progress: [hosting preparation](HOSTING.md), backup/restore tooling and release checks are prepared; public entry without a shared password is approved; paid provisioning and hosted/device acceptance remain outstanding. The user also requested a path to a large audience; [SCALING_PLAN.md](SCALING_PLAN.md) records staged follow-up work without treating the single-server playtest as production scale. Do not purchase or publish until the concrete deployment/access review.

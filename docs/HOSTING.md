...
Milestone 0.2.8 deployment preparation is in progress. No service, disk, account, domain or subscription has been created. There is no hosted playtest URL yet. The user approved starting this milestone and requested a path to a large audience. See [SCALING_PLAN.md](SCALING_PLAN.md); this initial deployment is for a small supervised playtest, with no measured public capacity claim.
...
On 2026-09-18, the published compute/storage estimate is **$7 + $0.25/month**, before bandwidth, build usage, workspace fees and taxes. Check the checkout total and usage allowances before creating paid resources. This is a starting proposal, not a capacity guarantee or approved purchase. [Render pricing](https://render.com/pricing), [compute plans](https://render.com/docs/compute-plans).
...
2. In Render, choose **New → Blueprint** and connect this repository and inspect every proposed resource and price before accepting. Importing the Blueprint provisions resources and may bill the account, and a successful deployment opens public access. Verify manual deployments remain selected.

...
**Next-version request, pending rule clarification:** Add per-player card resets with a confirmation, remaining-use count, and disabled unavailable state. Requested limits are once a day while guessing and three times during setup. Resolve the reset meaning and allowance boundaries in GAME_DESIGN.md before implementing. Persist approved allowances so refresh/reconnect cannot replenish them; test cancellation, exhausted limits, duplicate requests and phase changes. Keep this separate from the already deployed room-mode release.

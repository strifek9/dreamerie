...
export default function RoomHistory({ days, roster, complete = false, total, ranked = false }: { days: DayReview[]; roster: Player[]; complete?: boolean; total: number; ranked?: boolean }) {
  return <section className="room-history" aria-label="Your revealed Dreams">
    ...
    {days.map((day) => <details className="recap-round" key={day.day} open={complete || undefined}>
      ...
      <DreamRoundReveal reveal={day.reveal} roster={roster} showRecognition ranked={ranked} headingLevel="h3" />
    </details>)}

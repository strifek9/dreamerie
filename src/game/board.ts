...
import type { Card, DreamWeek, GuessingBoardView, GuessingRound, Player, PlayerId } from './types.ts'
...
export function createGuessingBoard(
  ...
  if (eligible.length < 4) throw new Error('Not enough unseen images to open this dream. Four are needed.')
  const decoys = shuffle(eligible, random).slice(0, 4)
  const cardIds = shuffle([...actual, ...decoys], random)
  const round: GuessingRound = { ...prepared, conceptId, id: `round-${week.id}-${roundIndex + 1}`, cardIds, assignments: new Map() }

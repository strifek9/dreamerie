...
export default function DreamRoundReveal({ reveal, headingLevel = 'h2', showImages = true, showRecognition = false, compact = false, roster }: { reveal: RoundRevealView; headingLevel?: 'h2' | 'h3'; showImages?: boolean; showRecognition?: boolean; compact?: boolean; roster?: readonly Player[] }) {
    ...
    <div className={`round-results${compact ? ' round-results-summary' : ''}`}>
      ...
      {reveal.guesses.map(({ player, chosen, actual, correct, clue }) => (
        <section className="friend-result" key={player.id} aria-label={`${player.name}'s result`}>
          ...
          <p className="result-message">{correct ? 'You remembered.' : 'The dream escaped you.'} <span>{correct && !reveal.missed ? '+1 point' : '0 points'}</span></p>
      ...
      {reveal.receivedGuesses.length > 0 && <section className="received-guesses" aria-label="Friends' guesses about your Dream">
        ...
        {showRecognition && <p className="recognition-score">
          {reveal.missed ? 'A missed day earns no recognition points.' : reveal.recognitionPoints > 0 ? `+${reveal.recognitionPoints} ${reveal.recognitionPoints === 1 ? 'point' : 'points'} for your Dream.` :

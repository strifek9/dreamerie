...
export const gameCommandSchema = {
  ...
  properties: {
    ...
    action: { oneOf: [action('start', { mode: { enum: ['classic', 'personal'] } }), action('close', { confirmed: { const: true } }, ['confirmed']), action('save', { conceptId: concept, cardId: card, clue: { type: 'string', minLength: 1, maxLength: 80 } }, ['conceptId', 'cardId']),
